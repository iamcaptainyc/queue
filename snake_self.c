...
void inisnake()
{
	...
	for(i=1;i<4;i++)
	{
		if((head=(snake *)malloc(sizeof(snake)))==NULL)//程序为头插法，此语句使head始终保持在蛇头部，下方tail相当于临时指针，不保存尾指针 
		{
			fprintf(stderr,"error in malloc!");
			exit(1);
		}
...
void creatfood()
{
	...
	q=head;
	snake *foods=NULL;
	if((foods=(snake *)malloc(sizeof(snake)))==NULL)
	{
		fprintf(stderr,"error in malloc!");
		exit(1);
	}
	...
	do{
		foods->x=rand()%56+2;
	}while((foods->x%2)!=0);//地图为水平30蛇身，即60像素，垂直30像素，因为水平上下边界各占一行，实际活动范围是1-29 
	foods->y=rand()%28+1; 
	while(q!=NULL)//检测食物是否出现在蛇身 
	{
		if(q->x==foods->x&&q->y==foods->y)
		{
			free(foods);
			flag=1;
			break;
...
void TheKey(snake **nexttail)
{
	*nexttail=head;
	TheWall();
    if(food->x!=(*nexttail)->x||food->y!=(*nexttail)->y)
	{
